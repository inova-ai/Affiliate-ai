...
test("Blob-backed motion transfer uses server-side Runway ephemeral uploads",async()=>{const s=await fs.readFile("server/index.js","utf8");assert.match(s,/sourceImagePath/);assert.match(s,/motionReferencePath/);assert.match(s,/headPrivate/);assert.match(s,/downloadPrivateToFile/);assert.match(s,/uploadEphemeral/);assert.match(s,/runway-ephemeral/);assert.match(s,/toFile/);assert.match(s,/Content-Type/);assert.match(s,/motion-transfer\/runway\/status/)});
