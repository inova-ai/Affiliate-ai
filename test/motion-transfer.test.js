...
test("Blob-backed motion transfer exposes signed HTTPS bridge URLs with HEAD metadata",async()=>{const s=await fs.readFile("server/index.js","utf8");assert.match(s,/sourceImagePath/);assert.match(s,/motionReferencePath/);assert.match(s,/headPrivate/);assert.match(s,/runwayBridgeUrl/);assert.match(s,/api\/runway-asset/);assert.match(s,/Content-Length/);assert.match(s,/Content-Type/);assert.match(s,/motion-transfer\/runway\/status/)});
