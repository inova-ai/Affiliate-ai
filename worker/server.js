...
function writeData(v,p){const m=String(v||"").match(/^data:.*?;base64,(.*)$/); if(!m) throw new Error("Invalid data URL"); fs.writeFileSync(p,Buffer.from(m[1],"base64"));}
...
app.post("/render",auth,async(req,res)=>{
 ...
 try{
  writeData(req.body.imageData,img);
  if(req.body.audioData) writeData(req.body.audioData,aud);
 ...
 }catch(e){console.error(e);res.status(500).json({jobId:id,status:"failed",error:e.message});}
